...
def visualize_perturbations(X_original, perturber, save_path=None):
    ...
    fig, axes = plt.subplots(
        len(perturbations), n_samples, figsize=(n_samples * 2, len(perturbations) * 2)
    )
    ...
    for i, (name, ptype, params) in enumerate(perturbations):
        ...
        for j in range(n_samples):
            ...
            if j == 0:
                ax.set_ylabel(
                    name, fontsize=12, rotation=0, ha="right", va="center", labelpad=50
                )
...
def plot_generalization_results(results_dict, save_path=None):
    ...
    for i, (model_name, results) in enumerate(results_dict.items()):
        ...
        ax1.bar(x + offset, results["accuracies"], width, label=model_name, alpha=0.8)

    ax1.set_ylabel("Accuracy", fontsize=12)
    ax1.set_title("Model Accuracy Under Different Perturbations", fontsize=14, pad=15)
    ax1.set_xticks(x)
    ax1.set_xticklabels(
        results_dict[list(results_dict.keys())[0]]["names"],
        rotation=45,
        ha="right",
        fontsize=9,
    )
    ...
    for i, (model_name, results) in enumerate(results_dict.items()):
        offset = width * (i - len(results_dict) / 2 + 0.5)
        bars = ax2.bar(
            x + offset, results["accuracy_drops"], width, label=model_name, alpha=0.8
        )
    ...
    ax2.set_title(
        "Accuracy Drop Magnitude (Positive=Degradation, Negative=Improvement)",
        fontsize=14,
        pad=15,
    )
    ax2.set_xticks(x)
...
def print_summary(results_dict):
    ...
    for model_name, results in results_dict.items():
        ...
        print(
            f"  Baseline accuracy (no perturbation): {baseline_acc:.4f} ({baseline_acc * 100:.2f}%)"
        )
        print(
            f"  Average accuracy (with perturbation): {avg_acc:.4f} ({avg_acc * 100:.2f}%)"
        )
...
def main():
    ...
    visualize_perturbations(
        X_test, perturber, save_path="results/perturbation_examples.png"
    )
    ...
    clf_standard = AdaBoostClassifier(
        estimator=DecisionTreeClassifier(
            max_depth=5
        ),  # Increase tree depth to ensure error < 0.5
        n_estimators=100,
        learning_rate=1.0,
        random_state=42,
    )
    ...
    clf_robust = RobustAdaBoost(
        base_estimator=DecisionTreeClassifier(
            max_depth=5
        ),  # Use deeper trees to ensure error < 0.5
        n_estimators=100,
        learning_rate=1.0,
        random_state=42,
    ...
    plot_generalization_results(
        results_dict, save_path="results/generalization_test.png"
    )
