...
def visualize_detailed_training(monitor, n_estimators, save_path=None):
    ...
    fig.suptitle(
        f"Detailed Training Analysis (n_estimators={n_estimators})",
        fontsize=16,
        fontweight="bold",
    )
...
def main():
    ...
    if noise_ratio > 0:
        print(
            f"噪声样本: {len(noise_idx)} ({len(noise_idx) / len(X_train) * 100:.1f}%)"
        )
        print(
            f"干净样本: {len(clean_idx)} ({len(clean_idx) / len(X_train) * 100:.1f}%)"
        )
    ...
    print(
        f"Base Estimator: Decision Tree (max_depth={config['base_estimator'].max_depth})"
    )
    print(f"Test Points: {len(config['n_estimators_list'])}")
    print(
        f"Estimators Range: {config['n_estimators_list'][0]} - {config['n_estimators_list'][-1]}"
    )
    ...
    if is_data_noisy:
        ...
        if len(monitor.noisy_weight_history) > 0:
            ...
            weight_ratio = (
                final_noisy_weight / final_clean_weight if final_clean_weight > 0 else 0
            )
