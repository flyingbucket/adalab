...
def visualize_digit_as_3d_surface(digit_image, title="手写数字3D高度图"):
    ...
    # 添加坐标轴
    mlab.axes(surf, color=(0, 0, 0), xlabel="X", ylabel="Y", zlabel="像素强度")
...
def visualize_multiple_digits_3d(X_data, y_data, n_samples=5):
    ...
    for i in range(min(n_samples, 10)):
        ...
        surf = mlab.surf(
            x + offset_x, y + offset_y, z * 2, colormap="jet", representation="surface"
        )
...
def visualize_feature_space_3d(X_data, y_data, n_samples=500):
    ...
    mlab.title("MNIST特征空间3D可视化 (PCA)", size=0.3, height=0.95, color=(0, 0, 0))

    mlab.show()


def visualize_weight_evolution_3d(
    weight_history, noise_indices, title="样本权重演变3D"
):
    """
    在3D空间中可视化样本权重的演变过程
    ...
    if len(noise_in_range) > 0:
        for idx in noise_in_range:
            ...
            mlab.plot3d([idx] * n_rounds, y, z_vals, color=(1, 0, 0), tube_radius=0.3)
    ...
    for level in [0.3, 0.5, 0.7]:
        ...
        if mask.any():
            contour = mlab.contour3d(
                digit, contours=[threshold_value], opacity=0.3, colormap="cool"
            )
